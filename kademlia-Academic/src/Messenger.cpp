                            ...
                            (sockaddr*)&sender,&len)))
    {
        ...
        else
        {
            ...
            Message* m = new Message(*b1,ntohs(*(uint16_t*)(buffer+4)),
                                     (short)count-RESERVED_BYTES,     //数据的长度就是接受到的总长度减去保留字段
                                     (uint8_t*)(buffer+12),
                                     *(uint8_t*)(buffer+6));
...
int Messenger::init(std::queue<Message*>* q, int port_ho, bool isPrivate)
{
    ...
    if(!isPrivate)
    {
        ...
        if(curl)
        {
            ...
            curl_easy_setopt(curl, CURLOPT_WRITEDATA,(void*)myipstring);   //myipstring会WriteCallback回调函数的最后一个参数
...
void Messenger::sendMessage(const Node node, const Message& msg) const
{
    ...
    if(node!=me)
    {
        Logger::getInstance().logFile("ssnsf", LOGGER_OUTGOING,
