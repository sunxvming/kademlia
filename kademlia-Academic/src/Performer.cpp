            ...
            case RPC_FIND_NODE :
            {
                ...
                else //answer相应请求的应答
                {
                    ...
                    if(got != p->searchInProgress.end())
                    {
                        ...
                        Kbucket b(top->getData()+NBYTE);      //所有k-buket中的node
                        sn->addAnswer(sender, &b);
                        Node askto[ALPHA_REQUESTS];
                        int retval = sn->queryTo(askto);
                        if (retval > 0) //need to query somebody
                        {
                            Message msg = generate_find_node_request(&k);   // 还是请求自己的key
                            msg.setFlags(msg.getFlags() |
                                        (top->getFlags() &
                                         ~(RPC_MASK|FLAG_ANSWER)));      //RPC_FIND_NODE 或上 除了RPC和 FLAG_ANSWER
                            for (int i = 0; i < retval; i++)
                                messenger->sendMessage(askto[i],msg);
                        }
                        else if (retval == 0) //Kbucket ready   都是ACTIVE，不需要再查了
                        {
                            Kbucket res;
                            sn->getAnswer(&res);
